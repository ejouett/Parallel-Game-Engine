/* Eamon Jouett COMP 310 , runs on avg 9 frames per second now. Better than orignal 3.5
 * compile with gcc -o engine engine.c Wall -pthread -lm
 * Instructions
 *
...
struct rotatearguments { //create struct for arguments since pthread create can only take in one
	double *Rx;
	double *Ry;
	double *Rz;
...
void *rotate(void* rap){ //method for rotating
	struct rotatearguments *ra = (struct rotatearguments*) rap;
	 for(int p = (*ra).start; p < NPOLYS; p =  p+NUM_THREADS) {
	//printf("%dhey\n", p);
            for(int q = 0; q < 3; q++) { // 3 points per polygon
                ...
                matrix_vec_mult((*ra).Rx, 3, (double*) (&(*ra).polys[p].pt[q]), (double*)&rot_x);

                // Rotate current point around y-axis, stashing result in rot_y
                matrix_vec_mult((*ra).Ry, 3, (double*) (&rot_x), (double*)&rot_y);
                ...
                matrix_vec_mult((*ra).Rz, 3, (double*) (&rot_y), (double*)&rot_z);
            }
        }
	return NULL;
...
int main() {
    ...
    for(int frame = 0; frame < NROTATIONS; frame++) {
    	...
    	//Pthread_create(&th, NULL, rotate, (void*) &frame);
	//pthread_join(th, (void **) &rot_z);
	for (int i = 0; i < NUM_THREADS; i++){ //create 3 new threads
	    struct rotatearguments ra;
	    ra.Rx = Rx;
 	    ra.Ry = Ry;
	    ra.Rz = Rz;
	    ra.polys = polys;
	    ra.start = i;
	     pthread_create(&th[i], NULL, rotate, (void*) &ra);
	   	 
	}

	for (int i = 0; i < NUM_THREADS; i++){	//join each thread
    ...
    printf("drew %f frames per second\n", NROTATIONS/elapsed_time); //runs on avg 9 frames per second now
}
